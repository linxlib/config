...
func (c *Config) DumpTo(out io.Writer, format string) (n int64, err error) {
	...
	if serializer, ok = c.serializers[format]; !ok {
		err = errors.New("not exists/register serializer for the format: " + format)
		return
	}
	...
	encoded, err := serializer.Encode(c.data)
	if err != nil {
		return
	}
